...
# ----------------------------------------------------------
# 2. Data for each layer
# ----------------------------------------------------------

# Name that will appear in the legend
hist_scen_name <- "Historical (2005–2015)"

# Historical line: one scenario's values up to 2015
hist_line <- eu_long %>%
  filter(Scenario == "NECP_POLICY", Year <= 2015) %>%
  mutate(Scenario = hist_scen_name)

# Scenario order (for legend)
scen_levels <- c(
  hist_scen_name,
  "FF55_COST_OPT",
  "NECP_COST_OPT",
...
# Scenario trajectories (4 scenarios) 2005–2030
traj_data <- eu_long %>%
...
# NECP targets (horizontal line – only 2030)
...
FigS2 <- ggplot() +
  ...
  # NECP 2030 target horizontal line in each panel
  geom_hline(
    data = necp_targets,
    aes(yintercept = Emissions, group = Country),
    colour   = necp_target_col,
    linewidth = 0.6,
    linetype = "dashed"
  ) +
  # vertical dashed line at 2015
  geom_vline(xintercept = 2015, linetype = "dashed") +
  scale_colour_manual(values = scenario_cols, name = NULL) +
  facet_wrap(~ Country, ncol = 3, scales = "free_y") +
  labs(
    x = NULL,
    y = expression("MtCO"[2]),
    title = expression("National CO"[2] * " emissions")
  ) +
  theme_classic() +
  theme(
...
y_left_max <- ceiling(ymax_stack / 50) * 50   
y_left_min <- floor(ymin_stack / 50) * 50    
...
p4a_diff <- ggplot(df_stack_diff, aes(x = X, y = Diff, fill = Fuel)) +
  ...
  scale_fill_manual(values = fuel_cols, drop = FALSE) +
  labs(
    title    = "Change and share of renewable energy use",
    subtitle = "EU-27",
    x        = NULL,
    fill     = "Fuel"
  ) +
...
p4a_diff <- p4a_diff +
  ...
  annotate("text", x = mean(pos_2030), y = y_lab, label = "2030",
           vjust = 0.5, size = 6, colour = "gray30", fontface = "bold") +
  annotate("text", x = mean(pos_2050), y = y_lab, label = "2050",
           vjust = 0.5, size = 6, colour = "gray30", fontface = "bold")
